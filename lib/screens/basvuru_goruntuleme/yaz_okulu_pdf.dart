...
class YazOkuluBasvuruPdf extends StatefulWidget {
  final String id;
  const YazOkuluBasvuruPdf({Key? key, required this.id}) : super(key: key);

  @override
  _YazOkuluBasvuruPdfState createState() => _YazOkuluBasvuruPdfState(id);
}

class _YazOkuluBasvuruPdfState extends State<YazOkuluBasvuruPdf> {
  final String id;
  YazOkuluBasvuruModel yazOkuluBasvuruModel = YazOkuluBasvuruModel();
  YazOkuluBasvuruService _yazOkuluBasvuruService = YazOkuluBasvuruService();
  _YazOkuluBasvuruPdfState(this.id);
  @override
  void initState() {
    ...
    FirebaseFirestore.instance
        ...
        .get()
        .then((value) {
      this.yazOkuluBasvuruModel =
          YazOkuluBasvuruModel.fromMap(value.data());
      setState(() {
  ...
  Future<void> _createPdf() async {
    ...
    element = PdfTextElement(
        text: 'Ogrenci Ad : ${yazOkuluBasvuruModel.ogrenciAd}',
        font: styleFont
    );
    ...
    element = PdfTextElement(
        text: 'Basvuru Turu : ${yazOkuluBasvuruModel.ogrenciBolum}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
    ...
    element = PdfTextElement(
        text:
        'Basvurulan Fakulte : ${yazOkuluBasvuruModel.basvurulanUniversite}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
        bounds: Rect.fromLTWH(10, result.bounds.bottom + 10, 0, 0))!;
    element = PdfTextElement(
        text: 'Basvurulan Bolum : ${yazOkuluBasvuruModel.yazOkuluTarih}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
        bounds: Rect.fromLTWH(10, result.bounds.bottom + 10, 0, 0))!;
    element = PdfTextElement(
        text: 'Ogrenci Numarasi : ${yazOkuluBasvuruModel.ogrenciDanisman}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
    ...
    document.dispose();
    saveAndLaunchFile(bytes, '${yazOkuluBasvuruModel.ogrenciAd}.pdf');
  }

...
class CapBasvuruPdf extends StatefulWidget {
  final String id;
  const CapBasvuruPdf({Key? key, required this.id}) : super(key: key);

  @override
  _CapBasvuruPdfState createState() => _CapBasvuruPdfState(id);
}

class _CapBasvuruPdfState extends State<CapBasvuruPdf> {
  final String id;
  CapBasvuruService _capBasvuruService = CapBasvuruService();
  CapBasvuruModel capBasvuruModel = CapBasvuruModel();
  _CapBasvuruPdfState(this.id);
  @override
  void initState() {
    ...
    FirebaseFirestore.instance
        ...
        .get()
        .then((value) {
      this.capBasvuruModel =
          CapBasvuruModel.fromMap(value.data());
      setState(() {
      });
    });
  }
  ...
  Future<void> _createPdf() async {
    ...
    element = PdfTextElement(
        text: 'Ogrenci Ad : ${capBasvuruModel.ogrenciAd}',
        font: styleFont
    );
    ...
    element = PdfTextElement(
        text: 'Basvuru Turu : ${capBasvuruModel.ogrenciUniversite}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
    ...
    element = PdfTextElement(
        text:
        'Basvurulan Fakulte : ${capBasvuruModel.basvurulanFakulte}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
        bounds: Rect.fromLTWH(10, result.bounds.bottom + 10, 0, 0))!;
    element = PdfTextElement(
        text: 'Basvurulan Bolum : ${capBasvuruModel.basvurulanBolum}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
        bounds: Rect.fromLTWH(10, result.bounds.bottom + 10, 0, 0))!;
    element = PdfTextElement(
        text: 'Ogrenci Numarasi : ${capBasvuruModel.ogrenciNumarasi}',
        font: styleFont);
    element.brush = PdfBrushes.black;
    result = element.draw(
        page: page,
    ...
    document.dispose();
    saveAndLaunchFile(bytes, '${capBasvuruModel.ogrenciAd}.pdf');
  }
